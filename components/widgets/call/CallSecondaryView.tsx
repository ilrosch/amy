...
export default function CallSecondaryView({ stream, isMicro, isCamera, setLocalFullScreen }: CallSecondaryViewType) {
  const { top } = useSafeAreaInsets();

  if (!isCamera) return;

  return (
    <Pressable onPress={() => setLocalFullScreen((prev) => !prev)} style={[styles.box, { marginTop: top }]}>
      {!isMicro && (
